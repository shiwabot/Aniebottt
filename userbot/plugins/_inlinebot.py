    ...
    buttons.append(
        [
            custom.Button.inline(
               f"{Anie_emoji} 𝐁𝐀𝐂𝐊 {AuraX_emoji}", data=f"page({(max_pages - 1) if page == 0 else (page - 1)})"
            ),
            custom.Button.inline(
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query == "@AuraXUserbot":
            ...
            result = builder.article(
                "@Aniebotsupports",
                text="""**Hey! This is [AnieUserbot.](https://t.me/Aniebots) \nYou can know more about me from the links given below 👇**""",
                buttons=[
                    [
    ...
    async def page(event):
        ...
        await event.edit(
            f"**Legenday** [AnieUserbot](https://t.me/Aniebots) __Working...__\n\n**Number of modules installed :** `{len(CMD_HELP)}`\n**page:** {page + 1}/{veriler[0]}",
            buttons=veriler[1],
            link_preview=False,
        )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            Anie_alert = "Ho gya aapka? Kabse tapar tapar dabae jaa rhe h. Khudka bna lo na agr chaiye to. (C)AnieUserbot"
