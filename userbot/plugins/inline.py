...
if PMPERMIT_PIC is None:
    WARN_PIC = "https://telegra.ph/file/3a655bdae71bd8364ada2.jpg"
else:
...
if lang == "si":
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query.startswith("anie"):
            rev_text = query[::-1]
            ...
            result = builder.article(
                title="Stats",
                text=f"**Showing Stats For {DEFAULTUSER}'s Aniebots** \nNote --> Only Owner Can Check This \n(C) [Aniebots](https://github.com/Anieteam/Aniebots)",
                buttons=[
                    [custom.Button.inline("Show Stats ?", data="terminator")],
                    [Button.url("Developed By", "https://github.com/Anieteam")],
                    [Button.url("Support Chat❤️", "t.me/Aniebotsupports")],
                ],
            )
            await event.answer([result])
    ...
    async def rip(event):
        ...
        if event.query.user_id != bot.uid:
            text = f"Please Get Your Own Aniebots And Don't Waste My Resources"
            await event.answer(text, alert=True)
            return
    ...
    async def sed(event):
        ...
        buttons = paginate_help(0, CMD_HELP, "helpme")
        sed = f"""Aniebots Modules Are Listed Here !\n
        ...
        if event.query.user_id == bot.uid and query.startswith("Fire-X"):
            ...
            result = builder.article(
                ...
                buttons=[
                    ...
                    [
                        Button.url(
                            "Repo Here", "https://github.com/Anieteam/Aniebots"
                        )
                    ],
                    [Button.url("Join Channel ❤️", "t.me/https://t.me/Aniebots")],
                ],
            )
            await event.answer([result])
        elif event.query.user_id == bot.uid and query.startswith("**Hello"):
...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id != bot.uid:
        resultm = builder.article(
            title="Not Allowded",
            text=f"You Can't Use This Bot. \nDeploy Aniebots To Get Your Own Assistant, Repo Link [Here](https://github.com/Anieteam/Aniebots)",
        )
        await event.answer([resultm])
        return
    testinput = event.pattern_match.group(1)
